...
void __env_free_with_buffering(struct Env *e)
{
	...
	struct Frame_Info *temp = NULL;

	LIST_FOREACH(frame, &modified_frame_list)
	{
		if (frame->environment == e) {
			frame->isBuffered = 0;
	...
	LIST_FOREACH(frame, &free_frame_list)
	{
		if (frame->environment == e) {
			frame->isBuffered = 0;
			frame->environment = NULL;
	...
	// free the pages in the working set
	...
	for (int i = 0; i < 1024; i++) {
		if (e->env_page_directory[i] != 0) {
			uint32 *ptr_page_table;
			get_page_table(e->env_page_directory, (void *) (i << 22),
					&ptr_page_table);
