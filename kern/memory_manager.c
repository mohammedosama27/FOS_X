 ...
 */
 ...
 void __freeMem_with_buffering(struct Env* e, uint32 virtual_address, uint32 size)
 {
	...
	uint32 range = ROUNDUP((size + virtual_address), PAGE_SIZE);

	for (int i = start; i < range; i += PAGE_SIZE) {
		uint32 permissions1 = pt_get_page_permissions(e, i);
		//check if it is buffered
		if (permissions1 & PERM_BUFFERED) {
			//get its frame info
			ptr_frame_info = get_frame_info(e->env_page_directory, (void*) i, &ptr_page_table);

			//check if it is modified
			if (permissions1 & PERM_MODIFIED) {
				...
				//add it to the head of free frame list
				free_frame(ptr_frame_info);
			}//end of modified
			else {
				//set flags
				...
				//add it to head of free frame list
				free_frame(ptr_frame_info);
			}//end of not modified

			//clear entry -> this page is no longer exist
			pt_clear_page_table_entry(e, i);
		}//end buffered page
	}//end search

	 //remove working set pages
	 for(int i= virtual_address; i< range; i+= PAGE_SIZE){
		 uint32 size = e->page_WS_max_size;
		 while(size > 0){
			 if(ROUNDDOWN(e->ptr_pageWorkingSet[size-1].virtual_address, PAGE_SIZE) == i){
				 //unmap and remove the page
				 env_page_ws_clear_entry(e, size-1);
				 unmap_frame(e->env_page_directory, (void*) i);
			 }
			 size--;
		 }//end search in working set
	 }//end search

	//remove empty page tables
	for (int i = virtual_address; i < range; i += PAGE_SIZE * 1024) {
		uint32* page_ptr = NULL;
		get_page_table(e->env_page_directory, (void*) i, &page_ptr);

		if (page_ptr != 0) {
			bool flag = 1;
			for (int j = 0; j < 1024; j++) {
				//check if it is present
				if (page_ptr[j] & PERM_PRESENT) {
					flag = 0;
					break;
				}
			}
	...
	//remove all pages in the range from page file
	for (uint32 i = virtual_address; i < range; i += PAGE_SIZE) {
